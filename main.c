...
void formatfunc(cJSON *Bible_Data) {
   ...
   *
   * Returns a string formatted with the Bible Data
   */

  // Parse the JSON Object
  Bible_Data = cJSON_Parse(chunk.response);
  cJSON *random_verse_object =
      cJSON_GetObjectItemCaseSensitive(Bible_Data->child, "random_verse");

  const char *translation_data = cJSON_Print(Bible_Data->child);
  // printf("translation_data: %s\n", translation_data);

  // Translation Data Object
  cJSON *Version = cJSON_GetObjectItemCaseSensitive(Bible_Data->child, "name");
  const char *version = cJSON_Print(Version);

  // Random Verse Object Data
  const char *random_verse_data = cJSON_Print(Bible_Data->child->next);
  // printf("random_verse_data: %s\n", random_verse_data);

  // Book Name Data
  cJSON *book_Name =
      cJSON_GetObjectItemCaseSensitive(Bible_Data->child->next, "book");
  const char *book = cJSON_Print(book_Name);

  // Chapter Number Data
  cJSON *chapter_Number =
      cJSON_GetObjectItemCaseSensitive(Bible_Data->child->next, "chapter");
  const char *chapter_num = cJSON_Print(chapter_Number);

  // Verse Number Data
  cJSON *number =
      cJSON_GetObjectItemCaseSensitive(Bible_Data->child->next, "verse");
  const char *num = cJSON_Print(number);

  // Verse Text String Data
  cJSON *text =
      cJSON_GetObjectItemCaseSensitive(Bible_Data->child->next, "text");
